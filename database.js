...
if (contentCount === 0) {
  ...
  const homeFields = [
    ...
    ['service2_title','MAIN SERVICE'],['service2_name','SWAHILI SERVICE'],['service2_time','10:30 AM - 12:30 PM'],
...
const extraCats = ['Easter Gift', 'Christmas Gift', 'Clergy Day', 'M-Pesa', 'Mustard Seed', 'KAYO Enrollment', 'KAYO Day', 'Dorcas Kitty'];
...
// Add Dorcas Kitty to expenditure items if missing
try { db.prepare("INSERT INTO expenditure_items (name) VALUES ('Dorcas Kitty')").run(); } catch (e) {}
...
try { db.exec('ALTER TABLE petty_cash_items ADD COLUMN group_name TEXT DEFAULT \'\''); } catch (e) {}
try { db.exec('ALTER TABLE petty_cash_items ADD COLUMN has_description INTEGER DEFAULT 0'); } catch (e) {}
try { db.exec('ALTER TABLE petty_cash_items ADD COLUMN sort_order INTEGER DEFAULT 0'); } catch (e) {}

const pciCount = db.prepare('SELECT COUNT(*) AS c FROM petty_cash_items').get().c;
if (pciCount === 0) {
  const insert = db.prepare('INSERT INTO petty_cash_items (name, group_name, has_description, sort_order) VALUES (?, ?, ?, ?)');
  const items = [
    ['Fuel', '', 0, 1],
    ['Vicarage', '', 0, 2],
    ['Hospitality', '', 0, 3],
    ['Transport', '', 0, 4],
    ['Renewals', '', 0, 5],
    ['Lay Readers', 'Renewals', 0, 1],
    ['Church Elders', 'Renewals', 0, 2],
    ['Sunday School', 'Renewals', 0, 3],
    ['Kayo', 'Renewals', 0, 4],
    ['Kama', 'Renewals', 0, 5],
    ["Mother's Union", 'Renewals', 0, 6],
    ['Archdeaconry Kitty', '', 0, 6],
    ['Pastoral Expense', '', 1, 7],
    ['Stationeries and Printing', '', 1, 8],
    ['Wages', '', 1, 9],
    ['Sanitary Expenses', '', 0, 10],
    ['Tissue', 'Sanitary Expenses', 0, 1],
    ['Detergent', 'Sanitary Expenses', 0, 2],
    ['Soaps', 'Sanitary Expenses', 0, 3],
    ['Disinfectant', 'Sanitary Expenses', 0, 4],
    ['Hand Sanitizer', 'Sanitary Expenses', 0, 5],
    ['Trash Bags', 'Sanitary Expenses', 0, 6],
    ['Dorcas Kitty', '', 0, 11],
    ['Repair and Maintenance', '', 0, 12],
    ['Batteries', 'Repair and Maintenance', 1, 1],
    ['Cables', 'Repair and Maintenance', 1, 2],
    ['Light Bulbs', 'Repair and Maintenance', 1, 3],
    ['PVC Pipes', 'Repair and Maintenance', 1, 4],
    ['Paint', 'Repair and Maintenance', 1, 5],
    ['Nails & Screws', 'Repair and Maintenance', 1, 6],
    ['Tools', 'Repair and Maintenance', 1, 7],
    ['Water', '', 0, 13],
    ['Sadamte', 'Water', 0, 1],
    ['Bottled Water', 'Water', 0, 2],
  ];
  for (const item of items) insert.run(...item);
};
...
// Migration: add spouse_member_id column for linked spouse member records
try { db.exec('ALTER TABLE members ADD COLUMN spouse_member_id INTEGER REFERENCES members(id)'); } catch (e) {}
