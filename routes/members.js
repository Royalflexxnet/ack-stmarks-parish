...
router.get('/profile', (req, res) => {
  const member = db.prepare(`
    SELECT id, membership_no, username, full_name, gender, phone, marital_status, spouse_name, spouse_phone, spouse_member_id, children_info, fellowship, department, created_at
  ...
  // If linked to a spouse member record, include spouse member details
  ...
  // Also check if THIS member is a spouse linked to another member
  const primaryMember = db.prepare('SELECT id, membership_no, full_name, phone FROM members WHERE spouse_member_id = ?').get(req.memberId);
  if (primaryMember) {
...
router.get('/card', (req, res) => {
  const member = db.prepare(`
    SELECT id, membership_no, full_name, phone, fellowship, department, created_at
    FROM members WHERE id = ?
  `).get(req.memberId);
