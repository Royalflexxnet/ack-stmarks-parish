  ...
  try {
    ...
    const txn = db.transaction(() => {
      ...
      if (spouse_is_member && marital_status === 'Married') {
        ...
        const spouseHashed = require('bcryptjs').hashSync(spouse_password, 10);
      ...
      return { memberId: result.lastInsertRowid, membershipNo: String(nextNo), spouseMemberId, username };
    });

    const { memberId, membershipNo, spouseMemberId, username } = txn();
...
router.get('/members/:id', requirePermission('can_manage_members'), (req, res) => {
  const member = db.prepare(`
    SELECT id, membership_no, username, full_name, gender, phone,
           marital_status, spouse_name, spouse_phone, spouse_member_id, children_info,
  ...
  // Include linked spouse member info
  if (member.spouse_member_id) {
    const spouseMember = db.prepare('SELECT id, membership_no, full_name, phone, gender FROM members WHERE id = ?').get(member.spouse_member_id);
    if (spouseMember) {
      member.spouse_member = {
        id: spouseMember.id,
        membership_no: spouseMember.membership_no,
        full_name: spouseMember.full_name,
        phone: spouseMember.phone,
        gender: spouseMember.gender
      };
    }
  }

  // Check if this member is a spouse linked to another member
  const primaryMember = db.prepare('SELECT id, membership_no, full_name, phone FROM members WHERE spouse_member_id = ?').get(req.params.id);
  if (primaryMember) {
    member.linked_to_member = {
      id: primaryMember.id,
      membership_no: primaryMember.membership_no,
