...
function authenticateFinance(req, res, next) {
  ...
  console.log('AUTH DEBUG: url=' + req.url + ' query=' + JSON.stringify(req.query) + ' header=' + (header ? 'yes' : 'no'));
  const token = header ? header.split(' ')[1] : (queryToken || null);
...
router.get('/petty-cash-items', (req, res) => {
  const all = db.prepare('SELECT * FROM petty_cash_items ORDER BY sort_order, name').all();
  const groups = {};
  const standalone = [];
  ...
  res.json({ items: standalone, groups });
});

router.post('/petty-cash-items', requirePerm('can_manage_petty_cash'), (req, res) => {
  const { name, group_name, has_description } = req.body;
  if (!name) return res.status(400).json({ error: 'Name is required' });
  try {
    const result = db.prepare('INSERT INTO petty_cash_items (name, group_name, has_description) VALUES (?, ?, ?)').run(name, group_name || '', has_description ? 1 : 0);
    res.status(201).json({ id: result.lastInsertRowid, name });
  } catch (e) {
    if (e.message.includes('UNIQUE')) return res.status(409).json({ error: 'Item already exists' });
    throw e;
  }
});
...
router.delete('/petty-cash/:id', (req, res) => {
  if (req.financeUser.role !== 'finance_superadmin') return res.status(403).json({ error: 'Only superadmin can delete petty cash entries' });
  const result = db.prepare('DELETE FROM petty_cash WHERE id = ?').run(req.params.id);
...
// Petty Cash Report (server-side generated HTML for printing)
router.get('/petty-cash/report', requirePerm('can_view_reports'), (req, res) => {
  const { start_date, end_date, cheque_id } = req.query;
  const sd = start_date || new Date().getFullYear() + '-01-01';
  const ed = end_date || new Date().toISOString().split('T')[0];

  let sql = `SELECT p.*, u.full_name AS created_by_name FROM petty_cash p LEFT JOIN finance_users u ON p.created_by = u.id WHERE p.entry_date >= ? AND p.entry_date <= ?`;
  const params = [sd, ed];
  if (cheque_id) { sql += ' AND p.cheque_id = ?'; params.push(cheque_id); }
  sql += ' ORDER BY p.entry_date DESC, p.created_at DESC';
  ...
  // Group entries by category (first segment of description before " - ")
  const byCat = {};
  entries.forEach(e => {
    const cat = e.description.split(' - ')[0] || 'Other';
    if (!byCat[cat]) byCat[cat] = { entries: [], total: 0 };
    byCat[cat].entries.push(e);
  ...
  const catRows = Object.entries(byCat).sort((a, b) => a[0].localeCompare(b[0])).map(([cat, data]) =>
    '<tr style="background:#f5f5f5;font-weight:600"><td colspan="3">' + esc(cat) + '</td><td>' + fmt(data.total) + '</td></tr>' +
    data.entries.map(e => '<tr><td style="padding-left:2rem">' + formatDate(e.entry_date) + '</td><td>' + esc(e.description) + '</td><td>' + esc(e.created_by_name || '—') + '</td><td>' + fmt(e.amount) + '</td></tr>').join('')
  ).join('');
...
<head>
  <meta charset="UTF-8">
  <title>Petty Cash Report — ACK St. Mark's Parish - Malaa</title>
  <style>
    * { box-sizing:border-box; margin:0; padding:0; }
    body { font-family:'Segoe UI',sans-serif; padding:2rem; color:#333; }
    .header { text-align:center; margin-bottom:2rem; padding-bottom:1rem; border-bottom:3px double #28a745; }
    .header h1 { font-size:1.4rem; color:#28a745; }
    .header p { color:#666; font-size:0.9rem; }
    .summary { display:flex; gap:1rem; justify-content:center; margin-bottom:2rem; }
    .summary-box { padding:1rem 2rem; border-radius:8px; text-align:center; background:#d4edda; }
    .summary-box .label { font-size:0.8rem; color:#666; }
    .summary-box .value { font-size:1.5rem; font-weight:700; margin-top:0.3rem; color:#155724; }
    table { width:100%; border-collapse:collapse; margin-bottom:1.5rem; }
    th, td { padding:0.6rem 1rem; text-align:left; border-bottom:1px solid #eee; font-size:0.9rem; }
    th { background:#28a745; color:white; font-weight:600; }
    .total-row { font-weight:700; background:#f9f9f9; font-size:1rem; }
    .footer { text-align:center; margin-top:2rem; padding-top:1rem; border-top:1px solid #ddd; color:#999; font-size:0.8rem; }
    @media print { body { padding:1rem; } .no-print { display:none; } }
    .no-print { text-align:center; margin-bottom:1rem; }
    .no-print button { padding:0.6rem 2rem; font-size:1rem; background:#28a745; color:white; border:none; border-radius:6px; cursor:pointer; }
  </style>
</head>
<body>
  <div class="no-print"><button onclick="window.print()">Download PDF</button></div>
  <div class="header">
    <h1>ACK St. Mark's Parish - Malaa</h1>
    <p>Petty Cash Report — ${sd.replace(/-/g, '/')} to ${ed.replace(/-/g, '/')}</p>
  </div>
  <div class="summary">
    <div class="summary-box"><div class="label">Total Petty Cash</div><div class="value">${fmt(total)}</div></div>
  </div>
  <table>
    <thead><tr><th>Date</th><th>Description</th><th>Added By</th><th>Amount</th></tr></thead>
    <tbody>
      ${catRows || '<tr><td colspan="4" style="text-align:center;color:#999">No entries in this period.</td></tr>'}
...
router.delete('/development/:id', (req, res) => {
  if (req.financeUser.role !== 'finance_superadmin') return res.status(403).json({ error: 'Only superadmin can delete development entries' });
  const result = db.prepare('DELETE FROM development WHERE id = ?').run(req.params.id);
  if (result.changes === 0) return res.status(404).json({ error: 'Entry not found' });
  res.json({ success: true });
});
